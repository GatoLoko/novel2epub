...
origin = (
    "http://stabbingwithasyringe.home.blog/translations/harem-tales"
...
synopsis_text = """
I was a good looking prince when I was reborn, and because I could do indecent
things as much as I like, I decided to make a harem while traveling with a
beautiful female elf</p>
...
INCLUDE_PROLOGUE = 100
WEIRD_SUBDIR = 613
VOLUME1_EXTRA = range(700, 711)
VOLUME2_EXTRA = range(800, 811)


def genlist(start: int, end: int) -> list[str]:
    chapterlist = []
    for i in range(start, end + 1):
        if i == INCLUDE_PROLOGUE:
            url = (
                f"{origin}harem-tales-of-a-reincarnated-elf-prince-prologue-1/"
            )
            chapterlist.append(url)
            url = f"{origin}reincarnated-elf-prince-prologue-2-2/"
            chapterlist.append(url)
        if i in range(100, 105 + 1):
            url = (
                f"{origin}reincarnated-elf-prince-chapter-"
                f"{str(i)[1:].lstrip('0')}/"
            )
            chapterlist.append(url)
        elif i == WEIRD_SUBDIR:
            ...
            chapterlist.append(url)
        elif i in VOLUME1_EXTRA:
            url = (
                f"{origin}reincarnated-elf-prince-extra-volume-1-chapter-"
                f"{str(i)[1:].lstrip('0')}/"
            )
            chapterlist.append(url)
        elif i in VOLUME2_EXTRA:
            url = (
                f"{origin}reincarnated-elf-prince-extra-volume-2-chapter-"
                f"{str(i)[1:].lstrip('0')}/"
            )
            chapterlist.append(url)
        else:
            url = (
                f"{origin}reincarnated-elf-prince-volume-{str(i)[0]}"
                f"-chapter-{str(i)[1:].lstrip('0')}/"
            )
            chapterlist.append(url)
    return chapterlist
